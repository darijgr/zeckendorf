...
%TCIDATA{LastRevised=Thursday, February 22, 2018 22:25:09}
...
Notice that $k\geq1$ (since $S$ is nonempty). From $s_{1}\in S\subseteq
\mathbb{N}_{2}$, we obtain $s_{1}\geq2$ and thus $s_{1}-1\geq1$. Hence,
$f_{s_{1}-1}>0$.
...
s_{i+1}-1$ (because $s_{i}$ and $s_{i+1}$ are integers).}, so that
$f_{s_{i}+1}\leq f_{s_{i+1}-1}$ (since the Fibonacci sequence $\left(
f_{1},f_{2},f_{3},\ldots\right)  $ is weakly increasing). Thus,%
\begin{align*}
