...
%TCIDATA{LastRevised=Friday, February 23, 2018 10:35:37}
...
Then, there exists a finite lacunar subset $T$ of $\mathbb{N}_{2}$ such
...
Hence, $s + 1 \notin S \cup \left\{ t_1 \right\}$ for every
$s \in S \cup \left\{ t_1 \right\}$\ \ \ \ \footnote{\textit{Proof.}
Let $s \in S \cup \left\{ t_1 \right\}$. We must prove that
$s + 1 \notin S \cup \left\{ t_1 \right\}$.

If $s \in S$, then this follows immediately from
(\ref{pf.lem.3.notin1}). Thus, we can WLOG assume that $s \notin S$.
Assume this. Combining $s \in S \cup \left\{ t_1 \right\}$ with $s \notin S$,
we obtain $s \in \left(S \cup \left\{ t_1 \right\}\right) \setminus S
\subseteq \left\{ t_1 \right\}$. Therefore, $s = t_1$.
Hence, $s + 1 = t_1 + 1 \notin S \cup \left\{ t_1 \right\}$.
This proves $s + 1 \notin S \cup \left\{ t_1 \right\}$.}.
In other words, the set
$S\cup\left\{  t_{1}\right\}  $ is lacunar. Denoting this set $S\cup\left\{
t_{1}\right\}  $ by $Q$, we thus have shown that the set $Q$ is lacunar.
Clearly, $Q$ is a finite set (since $S$ is finite).
Moreover, $\left\{ t_1 \right\} \subseteq \mathbb{N}_2$ (since
$t_1 \in \mathbb{N}_2$), so that
$Q = \underbrace{S}_{\subseteq \mathbb{N}_2}
\cup \underbrace{\left\{ t_1 \right\}}_{\subseteq \mathbb{N}_2}
\subseteq \mathbb{N}_2 \cup \mathbb{N}_2 = \mathbb{N}_2$.
From $Q = S \cup \left\{ t_1 \right\}$, we obtain
$Q \setminus \left\{ t_1 \right\} = \left( S \cup \left\{ t_1 \right\} \right)
\setminus \left\{ t_1 \right\} = S$ (since $t_1 \notin S$).
Also, $t_1 \in \left\{ t_1 \right\} \subseteq S \cup \left\{ t_1 \right\} = Q$.
Therefore,
\begin{align*}
\sum\limits_{t\in Q}f_{t}
&  = \sum\limits_{t\in Q \setminus \left\{  t_{1}\right\} }f_{t} + f_{t_1}
= \underbrace{\sum\limits_{t\in S}f_{t}}_{=\nu-f_{t_{1}}}
+f_{t_{1}}
\ \ \ \ \ \ \ \ \ \ \left(  \text{since } Q \setminus \left\{ t_1 \right\} = S \right)  \\
&  = \left(\nu - f_{t_1}\right) + f_{t_1} = \nu.
\end{align*}
Hence, we have found a finite lacunar subset $Q$ of $\mathbb{N}_2$ such
that $\nu = \sum\limits_{t \in Q} f_t$.
Hence, there exists a finite lacunar subset $T$ of $\mathbb{N}_2$ such that
$\nu = \sum\limits_{t \in T} f_t$ (namely, $T = Q$).
This proves Lemma~\ref{lem.3} for the case $n=\nu$. This completes the
...
yields
$T=\varnothing$\ \ \ \ \footnote{\textit{Proof.} Assume the contrary.
Thus, $T \neq \varnothing$. Hence, $\sum\limits_{t\in T}f_{t}$ is a nonempty
sum of positive integers (since the Fibonacci numbers $f_t$ are positive),
and thus itself is a positive integer. Thus,
$\sum\limits_{t\in T}f_{t} > 0$. This contradicts
$\sum\limits_{t\in T}f_{t} = n = 0$. This contradiction shows that our assumption
was wrong, qed.}. Similarly, $T^{\prime
}=\varnothing$. Comparing this with $T = \varnothing$, we obtain
$T=T^{\prime}$. Hence, we have shown that Lemma~\ref{lem.4} holds for
...
But $f_{\max T}$ is an addend in the sum $\sum\limits_{t\in T}f_{t}$
(since $\max T \in T$). Since the Fibonacci
numbers $f_{t}$ are all nonnegative, we thus obtain
...
\setminus\left\{  \mu\right\}  $. Clearly, $S$ is a finite subset of
$\mathbb{N}_{2}$. Furthermore, $S$ is a subset of the lacunar subset $T$ of
$\mathbb{Z}$
(because $S=T\setminus\left\{  \mu\right\} \subseteq T$), and thus itself
is lacunar (since every subset of a lacunar subset of $\mathbb{Z}$ is lacunar).
Thus, $S$ is a finite lacunar subset of $\mathbb{N}_2$.
Similarly,
$S^{\prime}$ is a finite lacunar subset of $\mathbb{N}_{2}$.
Obviously, $\sum\limits_{t\in S}f_{t} \geq 0$ (since all $f_t$ are nonnegative).
Now,%
\begin{align*}
...
integer (because $\nu-f_{\mu}=\sum\limits_{t\in S}f_{t} \geq 0$)
and satisfies $\nu-f_{\mu}<\nu$ (because $f_{\mu
...
\begin{theorem}
[Zeckendorf theorem]\label{thm.5} Let $n \in \mathbb{N}$. Then, there
...
\begin{proof}
[Proof of Theorem~\ref{thm.5}.]There exists a
...
(because any two such subsets are equal (according to Lemma~\ref{lem.4})).
Thus, there exists \textbf{one and only one} finite lacunar subset $T$ of
...
$1-\phi^{-2}=\phi-1$.
Also, $\left(\phi-1\right)^n \geq 0$ (since $\phi-1 \geq 0$) and thus
$\dfrac{1}{\sqrt{5}} \left(\phi-1\right)^n \geq 0$.
...
\end{align}
(by (\ref{pf.thm.8.triangle1}), applied to
$u = \sum\limits_{t\in T}\left(  f_{m+a_{t}+1}-\phi f_{m+a_{t}%
}\right)$ and
$v = \sum\limits_{s\in S}\left(  f_{m+s+1}-\phi
f_{m+s}\right)$).
