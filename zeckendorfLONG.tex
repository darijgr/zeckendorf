...
%TCIDATA{LastRevised=Thursday, February 22, 2018 21:12:25}
...
\footnotetext{Here and in the following, $\max\varnothing$ is understood
to be $0$.}

\begin{remark}
\textbf{1)} Theorem~\ref{thm.1} generalizes the so-called \textit{Zeckendorf
family identities} (which correspond to the case when all $a_{t}$ are $=0$),
which were discussed in \cite{1}.\footnotemark
...
\footnotetext{The first seven of these identities
are%
\begin{align*}
1f_{n}  &  =f_{n}\text{ for all }n\geq1;\\
2f_{n}  &  =f_{n-2}+f_{n+1}\text{ for all }n\geq3;\\
