...
%TCIDATA{LastRevised=Thursday, February 22, 2018 20:48:41}
...
\textit{Proof of Lemma~\ref{lem.2}.} We WLOG assume that $S$ is nonempty (since
...
\textit{Proof of Lemma~\ref{lem.3}.} We are going to prove Lemma~\ref{lem.3} by strong induction
over $n$:
...
\textit{Proof of Lemma~\ref{lem.4}.} We are going to prove Lemma~\ref{lem.4} by strong induction
over $n$:
...
\textit{Proof of Theorem~\ref{thm.5}.} Fix some $n\in\mathbb{N}$. Then, there exists a
...
\textit{Proof of Theorem~\ref{thm.6}.} By Binet's formula,%
\[
...
\textit{Proof of Theorem~\ref{thm.7}.} Let $\psi=\phi-1$. It is easily seen that
...
S}\left(  \phi-1\right)  ^{s}\leq\phi-1$. This proves Theorem~\ref{thm.7}.
...
\textit{Proof of Theorem~\ref{thm.8}.} Let us define a real constant $C$ by
...
subset of $\mathbb{N}_{2}$. Hence, Theorem~\ref{thm.7} (applied to $S+m$ instead of $S$)
...
\textit{Proof of Theorem~\ref{thm.1}.} Then, there exists a finite lacunar subset $S$ of
$\mathbb{Z}$ such that
\[
\left(
\begin{array}
