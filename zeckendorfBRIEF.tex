...
%TCIDATA{LastRevised=Thursday, February 22, 2018 21:33:41}
...
\begin{remark}
\textbf{1)} The \textit{Zeckendorf family identities}\ from \cite{1} are the
result of applying Theorem~\ref{thm.1} to the case when all $a_{t}$ are $=0$.
...
2,4,6,\ldots\right\}  }\left(  \phi-1\right)  ^{s}=\sum_{t\in\left\{
1,2,3,\ldots\right\}  }\left(  \phi-1\right)  ^{2t}=\phi-1
\]
