...
%TCIDATA{LastRevised=Thursday, February 22, 2018 20:48:34}
...
\end{align*}
} from \cite{1} are the result of applying Theorem~\ref{thm.1} to the case when all
$a_{t}$ are $=0$.
...
\begin{proof}
This is rather clear either by a telescoping sum argument
...
\begin{proof}
Induction over $n$. The main idea here is to let $t_{1}$ be
the maximal $\tau\in\mathbb{N}_{2}$ satisfying $f_{\tau}\leq n$, and apply
Lemma~\ref{lem.3} to $n-t_{1}$ instead of $n$. The details are left to the reader (and
can be found in \cite{this.long}).
\end{proof}
...
\begin{proof}
Induction over $n$. Use Lemma~\ref{lem.2} to show that $\max T<\max
...
\begin{proof}
Binet's formula yields $f_{n}=\dfrac{\phi^{n}-\phi^{-n}}%
...
\begin{proof}[Proof of Lemma~\ref{lem.7}.] Since $S$ is a lacunar subset of $\mathbb{N}_{2}%
...
identities] \label{thm.8} Let $T$ be a finite set, and $a_{t}$ be an integer for every
$t\in T$.
...
\begin{proof}[Proof of Theorem~\ref{thm.8}.] Choose a high enough integer $N$. What exactly
...
\limits_{s\in S}f_{N+s}$. But Theorem~\ref{thm.8} is not proven yet: Theorem~\ref{thm.8} requires
...
conclude from Lemma~\ref{lem.4} that $S+n=S^{\prime}+n$, so that $S=S^{\prime}$, proving
Lemma~\ref{lem.9}.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm.1}.]
Now, Theorem~\ref{thm.1} is clear, since the existence follows from Theorem~\ref{thm.8} and the
uniqueness from Lemma~\ref{lem.9}.
\end{proof}
